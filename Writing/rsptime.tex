...
We collected the PRIs for the  Contiki example ``helloworld''. \Cref{HelloworldPri} shows the histogram of the PRIs, where values $\geq$12ms are collected at 12ms. The result shows that the majority of PRIs are clustered around 9.5ms, whilst a clear gap exists between the outliers, that is, the extended response time that could be exploited by the attack. 

The results suggests that for this device, CC2538, PING is normally responded in [9.0, 10.3]ms, which corresponds to the case of Sensor Node 1 in \Cref{FingerprintTheory}. We further plotted the outliers in \Cref{HelloworldPriOutliers}. We suppose the outliers are caused by PING request arriving when the processor was performing another task as described by the case of Sensor Node 2 in \Cref{FingerprintTheory}. The distribution shown in \Cref{HelloworldPriOutliers} is due to the fact that different tasks delays the PING request differently.
